...
* `org.csanchez.jenkins.plugins.kubernetes.KubernetesLauncher.templateResolutionTimeoutSeconds` : The time in seconds an agent launch waits for its (dynamic) pod template to become available after a controller restart before giving up. This avoids leaking the agent pod when the launcher runs before the pipeline has re-registered the template on resume (defaults to `60`)
